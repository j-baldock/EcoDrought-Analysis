...
Define focal basins
```{r}
focalbasins <- c("West Brook", "Paine Run", "Staunton River", "Big Creek", "Coal Creek", "McGee Creek", "Donner Blitzen")
```

Designate big and little g data:
```{r}
dat_day_big <- dat %>% filter(site_name %in% c("Donner Blitzen River nr Frenchglen NWIS", "BigCreekLower", "CoalCreekLower", "McGeeCreekLower", "West Brook NWIS", "Paine Run 10", "Staunton River 10"))

dat_day_little <- dat %>% filter(subbasin %in% focalbasins, site_name %in% unlist(siteinfo2 %>% filter(designation == "little") %>% select(site_name)))
...
```{r}
datatable(dat_day_join %>% group_by(site_name_little) %>% summarize(numdays = n()),
...
Designate big and little g data:
```{r}
dat_week_big <- dat_week %>% filter(site_name %in% c("Donner Blitzen River nr Frenchglen NWIS", "BigCreekLower", "CoalCreekLower", "McGeeCreekLower", "West Brook NWIS", "Paine Run 10", "Staunton River 10"))

dat_week_little <- dat_week %>% filter(subbasin %in% focalbasins, site_name %in% unlist(siteinfo2 %>% filter(designation == "little") %>% select(site_name)))
```
...
```{r}
datatable(dat_week_join %>% group_by(site_name_little) %>% summarize(numweeks = n()),
