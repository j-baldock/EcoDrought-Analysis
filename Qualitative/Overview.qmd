...
```{r}
jpeg("C:/Users/jbaldock/OneDrive - DOI/Documents/USGS/EcoDrought/EcoDrought Working/Presentation figs/Fig1_ClimateDensityDistributions.jpg", width = 3, height = 3, units = "in", res = 1000)
...
polygon(x = c(10, 1, 1, 10), y = c(-10, -10, 10, 10), border = NA, col = "white")
axis(1)
points(x = jitter(extracted_vals2$Im, factor = 20), y = rep(0, times = dim(extracted_vals2)[1]), pch = 21, bg = extracted_vals2$color, cex = 1.5)
...
axis(1)
points(x = jitter(extracted_vals2$Imr, factor = 20), y = rep(0, times = dim(extracted_vals2)[1]), pch = 21, bg = extracted_vals2$color, cex = 1.5)
...
polygon(x = c(10, 1, 1, 10), y = c(-10, -10, 10, 10), border = NA, col = "white")
axis(1)
points(x = jitter(extracted_vals2$fs, factor = 20), y = rep(0, times = dim(extracted_vals2)[1]), pch = 21, bg = extracted_vals2$color, cex = 1.5)

dev.off()



# df_legend_gath <- df_legend %>% select(Im, Imr, fs) %>% gather(key = "type", value = "value")
# ggplot() +
#   geom_density(data = df_legend_gath %>% filter(type == "Im"), aes(x = value), fill = "grey70") +
#   geom_point(data = extracted_vals2, aes(x = jitter(Im, factor = 10), y = 0), shape = 21, fill = extracted_vals2$color, size = 5) +
#   #geom_vline(data = extracted_vals2, aes(xintercept = jitter(Im, factor = 20)), color = extracted_vals2$color) +
#   theme_bw() + theme(panel.grid = element_blank(), 
#                      axis.text.y = element_blank(), 
#                      axis.ticks.y = element_blank()) +
#   opts(panel.border = theme_L_border()) +
#   xlab("Aridity") + ylab("Density")
# 
# p2 <- ggplot() +
#   geom_density(data = df_legend_gath %>% filter(type == "Imr"), aes(x = value), fill = "grey") +
#   geom_point(data = extracted_vals2, aes(x = jitter(Imr, factor = 10), y = 0), shape = 21, fill = extracted_vals2$color, size = 5) +
#   theme_bw() + theme(panel.grid = element_blank()) +
#   xlab("Seasonality") + ylab("Density")
# 
# p3 <- ggplot() +
#   geom_density(data = df_legend_gath %>% filter(type == "fs"), aes(x = value), fill = "grey") +
#   geom_point(data = extracted_vals2, aes(x = jitter(fs, factor = 10), y = 0), shape = 21, fill = extracted_vals2$color, size = 5) +
#   theme_bw() + theme(panel.grid = element_blank()) +
#   xlab("Precipition as snow") + ylab("Density")
# 
# egg
```
