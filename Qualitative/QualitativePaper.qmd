...
Generate fixed and time varying (by day of year) drought/low flow thresholds from long-term (1970-2025) big G streamflow data. Quantiles correspond to those used in Hammond *et al.* (2022): 0.30, 0.20, 0.10, 0.05, 0.02. 
```{r}
# calculate site-level fixed low flow thresholds
dat_clean_big_fixed <- dat_clean_big %>% 
  group_by(site_name, basin, subbasin, region) %>%
  summarize(thresh_30_fix = quantile(Yield_mm, probs = 0.3, na.rm = TRUE),
            thresh_20_fix = quantile(Yield_mm, probs = 0.2, na.rm = TRUE),
            thresh_10_fix = quantile(Yield_mm, probs = 0.1, na.rm = TRUE),
            thresh_5_fix = quantile(Yield_mm, probs = 0.05, na.rm = TRUE),
            thresh_2_fix = quantile(Yield_mm, probs = 0.02, na.rm = TRUE)) %>%
  ungroup()
(dat_clean_big_fixed)
...
dat_clean_big_variable <- dat_clean_big %>% 
  ...
  summarize(thresh_30_var = quantile(Yield_mm, probs = 0.3, na.rm = TRUE),
            thresh_20_var = quantile(Yield_mm, probs = 0.2, na.rm = TRUE),
            thresh_10_var = quantile(Yield_mm, probs = 0.1, na.rm = TRUE),
            thresh_5_var = quantile(Yield_mm, probs = 0.05, na.rm = TRUE),
            thresh_2_var = quantile(Yield_mm, probs = 0.02, na.rm = TRUE)) %>%
  ungroup()
...
dat_clean_little_join <- dat_clean %>% 
  ...
  mutate(month = month(date),
         year = year(date),
         designation = "little",
         drought_fix = ifelse(Yield_mm <= thresh_30_fix & Yield_mm > thresh_20_fix, "q30", 
                              ifelse(Yield_mm <= thresh_20_fix & Yield_mm > thresh_10_fix, "q20", 
                                     ifelse(Yield_mm <= thresh_10_fix & Yield_mm > thresh_5_fix, "q10",
                                            ifelse(Yield_mm <= thresh_5_fix & Yield_mm > thresh_2_fix, "q05", 
                                                   ifelse(Yield_mm <= thresh_2_fix, "q02", "none"))))),
         drought_var = ifelse(Yield_mm <= thresh_30_var & Yield_mm > thresh_20_var, "q30", 
                              ifelse(Yield_mm <= thresh_20_var & Yield_mm > thresh_10_var, "q20", 
                                     ifelse(Yield_mm <= thresh_10_var & Yield_mm > thresh_5_var, "q10",
                                            ifelse(Yield_mm <= thresh_5_var & Yield_mm > thresh_2_var, "q05", 
                                                   ifelse(Yield_mm <= thresh_2_var, "q02", "none")))))) %>%
  mutate(drought_fix = factor(ifelse(is.na(Yield_mm), NA, drought_fix), levels = c("none", "q30", "q20", "q10", "q05", "q02")),
         drought_var = factor(ifelse(is.na(Yield_mm), NA, drought_var), levels = c("none", "q30", "q20", "q10", "q05", "q02"))) 
...
heatmapfun_fix <- function(subbas, months, bigG) {
  ...
  p <- dd %>%
    ...
    scale_fill_viridis(option = "A", direction = -1, discrete = TRUE, limits = c("none", "q30", "q20", "q10", "q05", "q02")) +
...
dat_clean_join_deficit <- dat_clean_join %>%
  mutate(deficit_30_fix = ifelse(Yield_mm < thresh_30_fix, thresh_30_fix - Yield_mm, 0),
         ...
         deficit_5_fix = ifelse(Yield_mm < thresh_5_fix, thresh_5_fix - Yield_mm, 0),
         deficit_2_fix = ifelse(Yield_mm < thresh_2_fix, thresh_2_fix - Yield_mm, 0),
         ...
         deficit_5_var = ifelse(Yield_mm < thresh_5_var, thresh_5_var - Yield_mm, 0),
         deficit_2_var = ifelse(Yield_mm < thresh_2_var, thresh_2_var - Yield_mm, 0))
...
deficitplotfun <- function(subbas, bigG, months, wateryears, dropsites = NA) {
  ...
  dd <- defdur_month %>% 
    ...
    group_by(site_name, designation, WaterYear) %>%
    summarize(duration_30_fix_prop = sum(duration_30_fix) / sum(ndays),
              duration_20_fix_prop = sum(duration_20_fix) / sum(ndays),
              duration_10_fix_prop = sum(duration_10_fix) / sum(ndays),
              duration_5_fix_prop = sum(duration_5_fix) / sum(ndays),
              duration_2_fix_prop = sum(duration_2_fix) / sum(ndays)) %>%
    ungroup()
  ...
  # barplot 30th perc
  p30 <- dd %>% 
    ggplot(aes(x = factor(site_name, levels = (mysites)), y = duration_30_fix_prop)) +
    geom_bar(aes(fill = site_name), stat = "identity") +
    facet_wrap2(~WaterYear, ncol = 1, strip = strip_themed(background_x = elem_list_rect(fill = alpha(c("red", "blue"), 0.5)))) +
    ylim(0,1) +
    ylab("Days below low flow threshold (%JAS)") + ggtitle("30th perc.") +
    theme_bw() + 
  ...
  # barplot 20th perc.
  p20 <- dd %>% 
    ggplot(aes(x = factor(site_name, levels = (mysites)), y = duration_20_fix_prop)) +
    geom_bar(aes(fill = site_name), stat = "identity") +
    facet_wrap2(~WaterYear, ncol = 1, strip = strip_themed(background_x = elem_list_rect(fill = alpha(c("red", "blue"), 0.5)))) +
    ylim(0,1) +
    ylab("Days below low flow threshold (%JAS)") + ggtitle("20th perc.") +
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
          legend.position = "none", axis.title.x = element_blank(),
          panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  ...
  # barplot 5th perc.
  p05 <- dd %>% 
    ggplot(aes(x = factor(site_name, levels = (mysites)), y = duration_5_fix_prop)) +
    geom_bar(aes(fill = site_name), stat = "identity") +
    facet_wrap2(~WaterYear, ncol = 1, strip = strip_themed(background_x = elem_list_rect(fill = alpha(c("red", "blue"), 0.5)))) +
    ylim(0,1) +
  ...
  # barplot 2nd perc
  p02 <- dd %>% 
    ggplot(aes(x = factor(site_name, levels = (mysites)), y = duration_2_fix_prop)) +
    geom_bar(aes(fill = site_name), stat = "identity") +
    facet_wrap2(~WaterYear, ncol = 1, strip = strip_themed(background_x = elem_list_rect(fill = alpha(c("red", "blue"), 0.5)))) +
    ylim(0,1) +
  ...
  p_sds <- dd %>% 
    gather(duration_30_fix_prop:duration_2_fix_prop, key = "metric", value = "duration") %>%
    mutate(quant = as.numeric(gsub(".*?([0-9]+).*", "\\1", metric)) ) %>% 
    filter(designation == "little") %>%
    group_by(WaterYear, metric, quant) %>%
    summarize(sddur = sd(duration, na.rm = TRUE)) %>%
    ungroup() %>%
    ggplot(aes(x = quant, y = sddur, color = WaterYear)) +
    geom_point() +
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          legend.position = "top") + guides(shape = guide_legend(title = "Water year"))
...
##### Snake River
```{r fig.width=10, fig.height=5}
deficitplotfun(subbas = "Snake River", bigG = "Pacific Creek at Moran NWIS", months = c(7:9), wateryears = c(2021, 2020))
```
##### Shields River
```{r fig.width=10, fig.height=5}
