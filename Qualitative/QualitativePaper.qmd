...
myridgesfun <- function(subbas) {
  ...
  return(ggplot(data = td) +
  geom_density_ridges(data = td %>% filter(designation == "little"), aes(x = logYield, y = MonthName), alpha = 0.5,point_alpha = 0.2) +
  geom_line(data = td %>% filter(designation == "little"), aes(x = logYield, y = MonthName, group = site_name, color = site_name), orientation = "y", alpha = 0.3) +
  geom_point(data = td %>% filter(designation == "little"), aes(x = logYield, y = MonthName, group = site_name, color = site_name), alpha = 0.3) +
  geom_line(data = td %>% filter(designation == "big"), aes(x = logYield, y = MonthName, group = site_name), orientation = "y", alpha = 0.5) +
  geom_point(data = td %>% filter(designation == "big"), aes(x = logYield, y = MonthName, group = site_name), alpha = 0.5) +
...
myridgesfun("West Brook")
```
#### Paine Run
```{r fig.width=7, fig.height=7}
myridgesfun("Paine Run")
```
#### Staunton River
```{r fig.width=7, fig.height=7}
myridgesfun("Staunton River")
```
#### Big Creek
```{r fig.width=7, fig.height=7}
myridgesfun("Big Creek")
```
#### Coal Creek
```{r fig.width=7, fig.height=7}
myridgesfun("Coal Creek")
```
#### McGee Creek
```{r fig.width=7, fig.height=7}
myridgesfun("McGee Creek")
```
#### Snake River
```{r fig.width=7, fig.height=7}
myridgesfun("Snake River")
```
#### Shields River
```{r fig.width=7, fig.height=7}
myridgesfun("Shields River")
```
#### Duck Creek
```{r fig.width=7, fig.height=7}
myridgesfun("Duck Creek")
```
#### Donner Blitzen
```{r fig.width=7, fig.height=7}
myridgesfun("Donner Blitzen")
```
:::
