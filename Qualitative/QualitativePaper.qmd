...
dat_clean_sub <- bind_rows(
  ...
  group_by(site_name) %>%
  mutate(z_Yield_mm = scale(Yield_mm, center = TRUE, scale = TRUE)) %>%
  ungroup()
...
deficitplotfun <- function(subbas, bigG, months, wateryears, dropsites = NA) {
  ...
  p_sds <- dd %>% 
    ...
    ggplot(aes(x = quant, y = sddur, color = dur50, group = WaterYear)) +
    geom_point() +
...
# wateryears <- c(2020, 2021)
# dropsites <- c("Mitchell Brook")
# months <- c(7:9)
# 
# dd <- defdur_month %>% 
#     filter(subbasin == subbas | site_name == bigG, Month %in% months, WaterYear %in% wateryears, !site_name %in% dropsites) %>%
...
#     ggplot(aes(x = quant, y = sddur, color = dur25, group = WaterYear)) +
#     geom_point() +
#     scale_color_gradient(low = "blue", high = "red", limits = c(0,1)) +
#     stat_smooth() +
#     xlab("Low flow threshold (percentile)") + ylab("Among-site SD(duration)") +
...
# 
#   str(p_sds)
#   p_sds$data[[1]]["gradient"]
#   
#   layer_data(p_sds)
#   unique(layer_data(p_sds)[,1])
