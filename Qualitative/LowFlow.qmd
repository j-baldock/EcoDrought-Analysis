...
Random effect of basin on primary relationship
```{r}
cat("model {
...
sigma.sig.alpha ~ dunif(0, 100)
sigma.sig.beta ~ dunif(0, 100)
...
joineddefdur2 <- joineddefdur %>% filter(basin_num != 8)

jags.data <- list("nObs" = dim(joineddefdur2)[1], 
                  "D" = unlist(joineddefdur2[,57+8]), 
                  "W" = joineddefdur2$tyz_sum_perc,
