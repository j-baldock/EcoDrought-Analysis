...
scatter_basinwithcis <- ggplot() +
  ...
  geom_point(data = joineddefdur, aes(x = tyz_sum_perc, y = duration_20_fix_diff, fill = basin), 
             shape = 21, size = 2, position = position_jitter(seed = 1, width = 1)) +
  scale_fill_manual(values = mycols) +
  theme_bw() + theme(panel.grid = element_blank(), legend.position = "none") + #ylim(-60,60) +
  xlab("Regional water availability (%)") + ylab("Difference in low flow duration (days)")
...
sigmaplot <- ggplot() +
  ...
  scale_fill_manual(values = mycols) +
  theme_bw() + theme(panel.grid = element_blank()) + ylim(0,max(pi_upp)) +
  xlab("Regional water availability (%)") + ylab(expression(paste("Model standard deviation (", sigma, ")")))
...
jpeg("C:/Users/jbaldock/OneDrive - DOI/Documents/USGS/EcoDrought/EcoDrought Working/Presentation figs/DroughtDuration_sitelevel_WestBrook_2020-2021_updatebasinnoci.jpg", width = 6.5, height = 9, units = "in", res = 1000)
egg::ggarrange(heat1, scatter_basinnocis, heat2, sigmaplot, ggplot() + theme_void(), effectplot, nrow = 3, ncol = 2)
dev.off()
```
