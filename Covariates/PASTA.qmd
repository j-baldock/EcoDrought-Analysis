...
```{r}
airtemp <- bind_rows(airtemp_wb, airtemp_shen, airtemp_flat, airtemp_shields, airtemp_snake, airtemp_db)
...
#### Donner Blitzen
```{r echo=FALSE, fig.width=9, fig.height=9}
dat_join %>% filter(subbasin == "Donner Blitzen", !is.na(tempc), TempReliability == 1) %>% 
  ggplot() +
  geom_line(aes(x = datetime, y = tempc_air), color = "darkorange") +
  geom_line(aes(x = datetime, y = tempc), color = "dodgerblue2") +
  ylab("Temperature (deg. C)") +
...
```{r include=FALSE}
dat_join %>% filter(site_name %in% "Fish Creek NWIS") %>% select(datetime, tempc, tempc_air) %>% dygraph(main = "Staunton River 10") %>% dyRangeSelector() %>% dyAxis("y", label = "Stream temp. (deg C)") %>% dyAxis("y2", label = "Air temp. (deg C)") %>% dySeries("tempc", axis = "y", label = "Stream") %>% dySeries("tempc_air", axis = "y2", label = "Air")
```
...
dat2 <- dat_join %>% 
  ...
  select(subbasin, site_name, datetime, year, yday, hour, airTemperature, waterTemperature) %>%
...
pasta_derived %>% filter(site_name == focal_site, rSquared_air >= 0.7 & rSquared_water >= 0.7) %>% ggplot(aes(x = yday, y = amplitudeRatio)) + geom_point() + ggtitle(focal_site) + geom_smooth(method = "lm", formula = y ~ poly(x, 2)) + facet_wrap(~year)
```
...
```{r fig.width=8, fig.height=6}
pasta_derived %>% filter(site_name == focal_site, rSquared_air >= 0.7 & rSquared_water >= 0.7) %>% ggplot(aes(x = yday, y = phaseLag)) + geom_point() + ggtitle(focal_site) + geom_smooth() + facet_wrap(~year)
```
...
```{r fig.width=8, fig.height=6}
pasta_derived %>% filter(site_name == focal_site, rSquared_air >= 0.7 & rSquared_water >= 0.7, meanRatio >=0, meanRatio <= 2) %>% ggplot(aes(x = yday, y = meanRatio)) + geom_point() + ggtitle(focal_site) + geom_smooth() + facet_wrap(~year)
```
...
```{r fig.width=8, fig.height=6}
pasta_derived %>% filter(site_name == focal_site, rSquared_air >= 0.7 & rSquared_water >= 0.7) %>% ggplot(aes(x = amplitudeRatio, y = phaseLag)) + geom_point() + ggtitle(focal_site) + geom_smooth(method = "lm") + facet_wrap(~year)
```
...
Define subbasins
```{r}
mysubbasins <- c("West Brook", "Paine Run", "Staunton River", "Big Creek", "Coal Creek", "McGee Creek", "Shields River", "Snake River", "Donner Blitzen")
```
...
mysites <- unlist(siteinfo2 %>% filter(site_name %in% unique(pasta_derived$site_name)) %>% filter(subbasin == "West Brook") %>% arrange(desc(area_sqmi)) %>% select(site_name))
myplots <- list()
for (i in 1:length(mysites)) {
  myplots[[i]] <- pasta_derived %>% filter(site_name == mysites[i], rSquared_air >= 0.7 & rSquared_water >= 0.7) %>% ggplot(aes(x = yday, y = amplitudeRatio)) + geom_point() + ggtitle(mysites[i]) + geom_smooth(method = "lm", formula = y ~ poly(x, 2)) + facet_wrap(~year)
}
```
:::panel-tabset
###### WB0
