...
ndiff <- 100
QGvec <- seq(from = min(dat$yield_big_vol_log), to = max(dat$yield_big_vol_log), length.out = ndiff)
...
jags.data <- list("nObs" = dim(dat)[1], "nSites" = length(unique(dat$site_name_cd)), 
                  ...
                  "Qg" = dat$yield_little_vol_log, "Qg2" = dat$yield_little_vol_log, "QG" = dat$yield_big_vol_log, 
...
Perform model selection using leave-one-out cross-validation (LOO-CV). Does the site-aware model perform better than the site-agnostic model? Yes, much better
```{r}
# get log-likelihoods
loglik1 <- mod_0$BUGSoutput$sims.list$loglik
loglik2 <- mod_0$BUGSoutput$sims.list$loglik2
...
reff2 <- relative_eff(exp(loglik2), chain_id = c(rep(1,200), rep(2,200), rep(3,200), rep(4,200), rep(5,200), 
                                                 rep(6,200), rep(7,200), rep(8,200), rep(9,200), rep(10,200)))
...
x_seq <- seq(from = min(dat$yield_big_vol_log), to = max(dat$yield_big_vol_log), length.out = nvals)
...
for (k in 1:nsites) { 
  ...
  points(yield_little_vol_log ~ yield_big_vol_log, data = dat %>% filter(site_name_cd == k), col = mycols[k])
  }
...
# control panel
nsim <- dim(Mcmcdat_0)[1]
x_seq <- seq(from = min(dat$yield_big_vol_log), to = max(dat$yield_big_vol_log), length.out = ndiff)
...
jags.data <- list("nObs" = dim(dat)[1], "nSites" = length(unique(dat$site_name_cd)), 
                  "sites" = dat$site_name_cd, #"indev" = dat_wb2$isevent,
                  "Qg" = dat$yield_little_vol_log, "Qg2" = dat$yield_little_vol_log, "QG" = dat$yield_big_vol_log, 
                  "QGvec" = QGvec, "nDiff" = ndiff, "varx" = sd(dat$yield_little_vol_log)^2)

# parameters to monitor
jags.params <- c("alpha", "beta", "alpha.mu", "beta.mu", "alpha.sigma", "beta.sigma", 
                 "sig.alpha", "sig.beta", "sig.alpha.mu", "sig.beta.mu", "sig.alpha.sigma", "sig.beta.sigma", 
                 "ag.alpha", "ag.beta", "ag.sig.alpha", "ag.sig.beta", "rho", 
                 "diff", "predlg", "loglik", "loglik2", "mu", "Qg", "portfolio1", "portfolio3", "atten3", "attenObs", 
                 "VpObs", "VpScen1", "VpScen2", "VpScen3", "port1", "port2", "port3")
...
for (j in 1:nsim) { 
  pred_arr_Port1[j,] <- Mcmcdat_1[j, str_subset(colnames(Mcmcdat_1), pattern = "port1")] 
  pred_arr_Port2[j,] <- Mcmcdat_1[j, str_subset(colnames(Mcmcdat_1), pattern = "port2")] 
  pred_arr_Port3[j,] <- Mcmcdat_1[j, str_subset(colnames(Mcmcdat_1), pattern = "port3")] 
  }
for (j in 1:ndiff) { 
...
# VpScen1
plot(seq(from = 0.75, to = 2.25, length.out = ndiff) ~ QGvec, type = "n", xlab = "(log) volumetric yield at Big G", ylab = "Portfolio strength")
