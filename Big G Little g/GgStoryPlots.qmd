...
```{r}
mybasins <- unique(dat$basin)[-c(4)]
myplots <- list()
for (i in 1:length(mybasins)) {
  myplots[[i]] <- dat %>% filter(designation %in% c("little", "medium"), basin == mybasins[i]) %>% ggplot() + geom_line(aes(x = date, y = log(Yield_filled_mm_7))) + facet_wrap(~site_name)
}
```
::: panel-tabset
#### Flathead
...
View little and medium g time series data (yield), by basin. Interactive using DyGraph, use the handles below the x-axis to change the time frame.
```{r}
mybasins <- unique(dat$basin)[-c(4)]
myplots <- list()
for (i in 1:length(mybasins)) {
  myplots[[i]] <- dat %>% filter(designation %in% c("little", "medium"), basin == mybasins[i]) %>% mutate(logYield = log(Yield_filled_mm_7)) %>% select(date, site_name, logYield) %>% spread(key = site_name, value = logYield) %>% dygraph() %>% dyRangeSelector() %>% dyAxis("y", label = "ln(Yield, mm)") %>% dyLegend(show = "never")
}
```
::: panel-tabset
#### Flathead
...
### Spread Creek, Snake
WARNING!!! Yield at NWIS gages are an order of magnitude greater than at little g gages...something is wrong!
