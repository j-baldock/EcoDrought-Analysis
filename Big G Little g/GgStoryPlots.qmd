...
bigplotfun <- function(mybasin, mysubbasin, CY, little, big, super, supergyears, mymap) {
  ...
  dat_basin_sub <- dat %>% 
    filter(site_name %in% c(little, big), !is.na(Yield_filled_mm_7)) %>% 
  ...
  dat_basin_res <- dat %>% filter(site_name %in% little) %>% select(site_name, date, WaterYear, Yield_filled_mm_7) %>%
