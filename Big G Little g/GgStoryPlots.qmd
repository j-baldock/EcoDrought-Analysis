...
for (i in 1:length(mybasins)) {
  cat(paste0(mybasins[i]))
  print(myplots[[i]])
}
```

View little and medium g time series data (yield), by basin, interactive using DyGraph
```{r, results='asis'}
mybasins <- unique(dat$basin)[-c(4)]
myplots <- list()
for (i in 1:length(mybasins)) {
  myplots[[i]] <- dat %>% filter(designation %in% c("little", "medium"), basin == mybasins[i]) %>% mutate(logYield = log(Yield_filled_mm_7)) %>% select(date, site_name, logYield) %>% spread(key = site_name, value = logYield) %>% dygraph() %>% dyRangeSelector() %>% dyAxis("y", label = "ln(Yield, mm)") 
}
...
Write functions to generate flow story plots
```{r}
...
### EcoDrought Basin Maps {.tabset .tabset-pills}
